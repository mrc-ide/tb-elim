...
rows = ord(1:10,2);
disp(C3c(rows,[2,end]))
tbl_migr = C3c(ord(:,2),[2,end]);


% -------------------------------------------------------------------------
% --- Now bring all values into single table

save country_data tbl_old tbl_migr;


ff=figure; fs = 14; nctr = 20;

yplt = tbl_old.rat(1:nctr);
lbls = tbl_old.ctrs2(1:nctr);
subplot(1,2,1);
bar(yplt); 
set(gca,'XTick',1:nctr,'XTickLabel',lbls,'fontsize',fs);
ylabel({'Proportion contribution to','annual incidence, 2019'});
title('(A) Elderly');
xtickangle(90);

yplt = tbl_migr.prop(1:nctr);
lbls = tbl_migr.iso3(1:nctr);
subplot(1,2,2);
bar(yplt); 
set(gca,'XTick',1:nctr,'XTickLabel',lbls,'fontsize',fs);
ylabel({'Proportion contribution to','annual incidence, 2019'});
title('(B) Foreign-born');
xtickangle(90);

set(ff, 'Position', [669   438   521   519]);